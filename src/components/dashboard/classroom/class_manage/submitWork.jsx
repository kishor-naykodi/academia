...
export default function SubmitWork() {
  ...
  useEffect(() => {
    async function getUpdatedFileResult() {
      const results = await axios.get(
        `http://localhost:3001/api/classroom/getResultSubmission/${user._id}/${assignment._id}`
      );
  ...
  }, [setHasUploaded, setHasUploadfile, assignment._id]);
      ...
      <main style={{ margin: "7rem" }}>
        <div className="fJ1Vac">
          ...
          <div className="EE538">
            ...
            {assignment.file ? (
              <div className="btn-primary btn-see-attachment">
                <a
                  href={`http://localhost:3001/api/classroom/getFile/${assignment.file.key}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  see attachment
          ...
          <aside className="BiaLW">
            {/*make changes here */}
            {hasUploaded ? (
              <div className="handedin_container">
                <div className="handed_in">
                  Handed in <AttachmentIcon />
                </div>{" "}
                <div className="doneuploads">
                  Done <CloudDoneIcon />
                </div>{" "}
                <div className="seeyouruploads">
                  <a
                    className="seeyouruploads_href"
                    href={`http://localhost:3001/api/classroom/getFile/${hasUploadFile.file.key}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    See your uploaded File <DownloadForOfflineIcon />
                  </a>{" "}
                </div>
              </div>
            ) : (
